# Le fichier de déclaration - format v0.3
...
vérificateur NE DOIT PAS appliquer la déclaration d'une origine à une autre. **La même règle
vaut en cours de session : lorsqu'un agent suit une navigation, une redirection ou une
transition d'API vers une nouvelle origine, celle-ci est hors de la déclaration précédente à
moins de déclarer elle-même** - une transaction qui commence sur une origine couverte
n'emporte pas cette couverture vers les origines où elle aboutit.
...
personnalisée non associée comme non déclarée. « Données transmises par un agent » désigne
...
- `third_parties` - chaque entité qui voit des données transmises par un agent, avec un `role`
  (`processor`, `subprocessor`, `infrastructure`, `analytics`, `security`, `model_provider`),
  ce qu'elle voit, et où elle traite. **`identifier` est OBLIGATOIRE et constitue l'identité
  lisible par machine - un domaine stable ou un URI contrôlé par l'exploitant; `name` n'est
  que la présentation.** Les familles d'entreprises et les noms semblables (« Example AI »,
  « Example AI Analytics », un fournisseur racheté) rendent un simple nom invérifiable; un
  identifiant est ce qu'un vérificateur peut réellement comparer au trafic observé. « Voit »
  inclut les transferts côté serveur qu'aucun observateur externe du réseau ne peut
  surveiller - un site qui ne stocke rien mais transmet tout à un fournisseur de modèle n'a
  rien divulgué tant que ce fournisseur n'est pas nommé. Un tableau vide est une réponse
  valide et évaluable.
...
- `deletion` - un mécanisme réel et activable par une machine. L'essentiel est qu'il soit
  ...
  avoir la même origine que la déclaration qui l'annonce.** Un point de suppression d'une
  autre origine est le chemin d'exfiltration de capacité le plus net de ce format - un agent
  qui exercerait plus tard le chemin déclaré porterait son jeton de suppression vers l'origine
  que la déclaration a nommée - ce n'est donc pas une règle de style, c'est la frontière. Un
  vérificateur donne F à cette clause pour un point d'une autre origine. La déclaration doit
  aussi dire comment un appelant prouve son droit de supprimer ce qu'il demande (`auth`),
  parce qu'un point de suppression sans authentification est une arme pointée vers les propres
  utilisateurs du site. Exigences sur le point lui-même :
...
- `receipt_support` - indique si le site prend en charge les reçus par interaction. `false` est
  ...
  **Un reçu NE DOIT PAS reproduire le contenu qu'il décrit.** Il nomme des classes de données
  ...
  une autre interaction après coup. **Un reçu portant un `deletion_token` porte une capacité
  vivante, et les deux côtés le traitent ainsi** : le site garde le jeton hors de ses propres
  journaux et caches, l'agent le range dans son coffre de capacités protégé (voir les règles
  du registre dans le document principal), jamais dans un export en clair, et les
  intermédiaires qui mettent des reçus en cache sont un risque nommé que le caractère à usage
  unique et à courte durée du jeton existe pour contenir. Un futur reçu signé (une signature
  sur l'enregistrement canonique de l'interaction) est là où l'extension `signature`
  ci-dessous prend toute sa valeur.
...
- un jeton de suppression n'est valide qu'à l'origine qui l'a émis, que pour l'interaction
  qu'il nomme, que pour la suppression, à usage unique et à courte durée de vie;
...
- DOIT traiter **chaque valeur de déclaration comme une donnée contrôlée par un adversaire,
  jamais comme une instruction exécutable - dans les rapports, et tout autant en aval** : un
  `name`, un `purpose`, un `reason` ou une classe personnalisée peut finir dans une ligne de
  journal, un argument de shell, une requête, une URL, un export de tableur ou une étiquette
  de métriques, et c'est du texte non fiable dans chacun de ces endroits, pas seulement dans
  du HTML rendu.
...
- DEVRAIT, lors des sondes de rétention, rendre son trafic synthétique réellement difficile à
  identifier - charges aléatoires, cadence réaliste, points de vue et réseaux variés - et
  DEVRAIT revérifier la réapparition à intervalles espacés (immédiatement, après un jour,
  après une semaine ou plus) avant de conclure « non réapparu » : une rétention qui refait
  surface par un traitement en lot ou un export différé est invisible pour une sonde unique et
  immédiate. La méthodologie complète des canaris est son propre document sur la feuille de
  route; ces planchers tiennent d'ici là.
- DOIT, devant un fichier d'un format antérieur, le dire dans sa sortie et évaluer selon les
  règles plus faibles de ce format - « v0.1 : expires et deletion.auth absents, évalué selon
  les règles v0.1 » - jamais promouvoir ni rétrograder silencieusement une déclaration d'un
  format à l'autre.
...
l'absence de rétention transformée.** Un site peut supprimer les mots et garder un vecteur, une
...
déclaration avec sa propre entrée de conservation, pourquoi des sondes de reconnaissance (le
site se *comporte-t-il* comme s'il se souvenait) font partie de la feuille de route de
vérification aux côtés du rejeu exact, et pourquoi un test de réapparition propre est une
preuve pour `content`, jamais pour `derived`.
...
cette chaîne; ensemble, ils les coupent tous. Les autres chaînes connues : une infrastructure
CDN partagée faisant passer deux origines pour une seule (fermée par la portée d'origine
exacte et la règle de transition en cours de session), et un vérificateur transformé en arme
par les URL et valeurs qu'une déclaration lui fournit (fermée par les règles pour
vérificateurs ci-dessus).
...
- Gestion des versions et migration. La v0.3 est un changement incompatible avec la grammaire
  de la v0.2 (`retention` est devenue une table de durées par classe,
  `third_parties[].identifier` est devenu obligatoire, `custom_classes` a été ajouté), fait
  alors que la norme n'a aucun adoptant - le moment honnête pour le faire. Les fichiers v0.1
  restent lisibles; les vérificateurs évaluent les formats antérieurs selon leurs propres
  exigences plus faibles et notent ce qui manque.

## Statut

Ébauche v0.3, publiée pour commentaires en parallèle avec le document principal de la norme.
